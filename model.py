...
from lstm_layers import LSTMLearnerLayer
        ...
        with tf.device('/gpu:0'):
            with tf.variable_scope('hintbot') as scope:
                ...
                with tf.variable_scope('BiGRU'):
                    ...
                    # hidden_states=tf.reshape(hidden_states,[200,2,20,56])
                ...
                with tf.variable_scope('LSTM'):
                    ...
                    hidden_states=tf.reshape(hidden_states,(-1,10,10))
                    self.outputs1_temporal, self.lstm_learner = \
                        self.lstm_propogation(inputs=hidden_states, input_dim=10,
                                              num_time_steps=7)
                ...
                with tf.variable_scope('mmsa'):
                    ...
                    hidden_states=tf.reshape(state1,(196,50,7,64))
                    O_seq = Attention(hidden_states, hidden_states, hidden_states,8,16)
                    # x_vector=tf.transpose(tf.stack(x_vector), [1, 0, 2])
                    # O_seq = Attention(x_vector, x_vector, x_vector,8,16)
